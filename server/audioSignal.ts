export interface DecodedPcm16MonoWav {
...
export interface AudioSignalComparison {
  input: AudioSignalMetrics;
  output: AudioSignalMetrics;
...
type NumericSamples = ArrayLike<number> | null | undefined;

export function decodePcm16MonoWav(audio: unknown): DecodedPcm16MonoWav {
  ...
  let data: WavDataChunk | undefined;
...
export function encodePcm16MonoWav(samples: NumericSamples, sampleRate: number): Buffer {
...
export function measureAudioSignal(samples: NumericSamples, sampleRate: number, frameMs = 20): AudioSignalMetrics {
  const source = samples instanceof Float32Array ? samples : Float32Array.from(samples || []);
  if (source.length === 0) {
    return {
      durationMs: 0,
...
export function compareAudioSignals(
  input: NumericSamples,
  output: NumericSamples,
  sampleRate: number
): AudioSignalComparison {
