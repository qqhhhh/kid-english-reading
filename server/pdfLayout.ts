...
function toPdfData(buffer: Uint8Array): Uint8Array {
  return new Uint8Array(buffer);
}

function cleanLayoutText(value: unknown = ""): string {
  return String(value || "")
    .replace(/\u0000/g, "")
    .replace(/\u00a0/g, " ")
    .replace(/\s+/g, " ")
...
function getTextItemGeometry(item: PdfTextItemLike, viewport: PdfViewportLike) {
...
function isEnglishTextItem(item: PdfLayoutItem): boolean {
  return /[A-Za-z]{3}/.test(cleanLayoutText(item?.text));
}

function isOverlaidAlternative(kept: PdfLayoutItem, candidate: PdfLayoutItem): boolean {
...
export function resolveOverlaidTextItems(items: readonly PdfLayoutItem[]): {
  items: PdfLayoutItem[];
  overlays: PdfOverlayAlternative[];
} {
  const keptItems: PdfLayoutItem[] = [];
  const overlays: PdfOverlayAlternative[] = [];
...
function mergeItemIntoLine(line: WorkingLayoutLine, item: PdfLayoutItem): void {
...
function canShareLine(line: WorkingLayoutLine, item: PdfLayoutItem, pageWidth: number): boolean {
...
function joinLineItems(items: readonly PdfLayoutItem[], lineHeight: number): string {
  const orderedItems = [...items].sort((a, b) => a.x - b.x);
  let text = "";
  let previous: PdfLayoutItem | null = null;
...
function groupItemsIntoLines(items: readonly PdfLayoutItem[], pageWidth: number): PdfLayoutLine[] {
  const sortedItems = [...items].sort((a, b) => a.y - b.y || a.x - b.x);
  const lines: WorkingLayoutLine[] = [];

  for (const item of sortedItems) {
    let targetLine: WorkingLayoutLine | null = null;
...
export async function extractPdfLayout(buffer: Uint8Array): Promise<PdfLayout> {
