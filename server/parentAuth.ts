...
import type { AccessSession, ParentAccessSession, ReviewAccessSession } from "./types/auth.js";
import type { ChildDeviceSessionRecord, ParentSessionRecord, ParentUserRecord, RegistrationKeyRecord } from "./types/data.js";
import {
...
interface AppendHeaderResponse {
  append(name: string, value: string): unknown;
}

interface PlatformAdminSessionLike {
  kind?: string;
  username?: string;
  role?: string;
}

interface RegistrationKeyOptions {
  label?: string;
  note?: string;
  maxUses?: number;
...
export function isPlatformAdminSession(session: PlatformAdminSessionLike | null | undefined) {
...
export function createRegistrationKey({
  ...
  batchId = "",
  createdByUserId = ""
}: RegistrationKeyOptions = {}): RegistrationKeyRecord & { key: string } {
...
export async function registerParent({ registrationKey, username, password, householdName = "" }: {
  ...
  try {
    ...
    if (String(error instanceof Error ? error.message : "").includes("UNIQUE constraint failed: parent_users.username")) {
...
export async function authenticateParent(username: unknown, password: unknown): Promise<ParentUserRecord | null> {
  const normalizedUsername = normalizeUsername(username);
  const validationError = validateParentCredentials(normalizedUsername, password);
...
export function createParentSession(userId: string) {
...
function readCookieValues(request: CookieRequest, name: string) {
...
function sessionCookie(name: string, value: string, maxAge: number, { includeDomain = true }: { includeDomain?: boolean } = {}) {
...
function sessionCookiesWithLegacyCleanup(name: string, value: string, maxAge: number): string | string[] {
...
function clearedSessionCookies(name: string): string | string[] {
...
export function readParentSession(request: CookieRequest): ParentSessionRecord | null {
...
export function revokeParentSession(request: CookieRequest) {
  for (const token of readCookieValues(request, sessionCookieName)) {
    revokeAuthSessionByTokenHash(sha256(token));
  }
}

export function setParentSessionCookie(response: HeaderResponse, token: string) {
  response.setHeader("Set-Cookie", sessionCookiesWithLegacyCleanup(sessionCookieName, token, sessionDurationSeconds));
}

export function clearParentSessionCookie(response: HeaderResponse) {
  response.setHeader("Set-Cookie", clearedSessionCookies(sessionCookieName));
}

export function publicParentSession(session: ParentUserRecord | ParentSessionRecord | ParentAccessSession | null | undefined) {
...
export function pairChildDevice({ code, label = "" }: { code: unknown; label?: string }) {
...
export function readChildSession(request: CookieRequest): ChildDeviceSessionRecord | null {
...
export function readAccessSession(request: CookieRequest): AccessSession | null {
...
export function readReviewSession(request: CookieRequest): ReviewAccessSession | null {
  const now = Date.now();
...
export function setReviewSessionCookie(response: AppendHeaderResponse) {
  const token = randomBytes(32).toString("base64url");
  const now = Date.now();
...
export function clearReviewSessionCookie(response: AppendHeaderResponse) {
  const cookies = clearedSessionCookies(reviewSessionCookieName);
  for (const cookie of Array.isArray(cookies) ? cookies : [cookies]) response.append("Set-Cookie", cookie);
}

export function revokeChildSession(request: CookieRequest) {
  for (const token of readCookieValues(request, childSessionCookieName)) {
    revokeChildDeviceSessionByTokenHash(sha256(token));
  }
}

export function setChildSessionCookie(response: AppendHeaderResponse, token: string) {
  const cookies = sessionCookiesWithLegacyCleanup(childSessionCookieName, token, childSessionDurationSeconds);
  for (const cookie of Array.isArray(cookies) ? cookies : [cookies]) response.append("Set-Cookie", cookie);
}

export function clearChildSessionCookie(response: AppendHeaderResponse) {
  const cookies = clearedSessionCookies(childSessionCookieName);
  for (const cookie of Array.isArray(cookies) ? cookies : [cookies]) response.append("Set-Cookie", cookie);
}

export function publicAccessSession(session: AccessSession | ParentSessionRecord | null | undefined) {
  if (!session) return null;
  if (("kind" in session && session.kind === "parent") || "username" in session) return publicParentSession(session);
  return {
    kind: "child",
    ...
    ...("kind" in session && session.kind === "review" ? { reviewOnly: true } : {})
  };
}
