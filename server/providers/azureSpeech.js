...
function azureTicksToMilliseconds(value) {
  return round(finiteNumber(value) / azureTickPerMillisecond, 2);
}

function mapAzureErrorType(errorType) {
...
function normalizeAzurePhone(phone, wordMatchTag) {
...
export function normalizeAzureResult(payload) {
  ...
  return {
    ...
    RecognizedText: String(best.Display || best.Lexical || payload.DisplayText || ""),
...
function parseAzureJsonResult(result) {
...
export async function assessWithAzure({ audio, referenceText }) {
  if (!audio?.length) {
