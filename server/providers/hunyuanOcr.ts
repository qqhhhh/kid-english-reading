...
interface HunyuanInstallationStatus {
  installed: boolean;
  controllable: boolean;
  paths: HunyuanRuntimePaths;
}

interface HunyuanRequestOptions {
  image: Uint8Array;
  model?: string;
  maxTokens?: number;
}

interface HunyuanRequestBody {
  model: string;
  temperature: number;
  max_tokens: number;
  stream: boolean;
...
function runtimePaths(): HunyuanRuntimePaths {
...
async function installationStatus(): Promise<HunyuanInstallationStatus> {
...
export function buildHunyuanOcrRequest({
  image,
  model = defaultModel,
  maxTokens = 4096
}: HunyuanRequestOptions): HunyuanRequestBody {
  return {
    model,
    temperature: 0,
...
function responseMessage(body: unknown): string {
  ...
  if (Array.isArray(content)) {
    return content.map((item) => {
      const record = (item && typeof item === "object" ? item : {}) as { text?: unknown; content?: unknown };
...
export function normalizeHunyuanOcrText(value: unknown): RecognizedOcrText {
...
export async function probeHunyuanOcr({
  fetchImpl = fetch,
  timeoutMs = 1500
}: HunyuanFetchOptions = {}): Promise<boolean> {
...
export async function getHunyuanOcrStatus({ fetchImpl = fetch }: HunyuanFetchOptions = {}): Promise<OcrProviderStatus> {
...
function getHunyuanPort(): number {
  const endpoint = endpointUrl();
  return Number(endpoint.port || (endpoint.protocol === "https:" ? 443 : 80));
}

async function findHunyuanServerPid(port: number): Promise<number | null> {
...
async function assertHunyuanServerProcess(pid: number): Promise<void> {
...
async function waitForHealth(expectedOnline: boolean, timeoutMs = 25000): Promise<boolean> {
...
export async function startHunyuanOcrService(): Promise<OcrProviderStatus> {
...
export async function stopHunyuanOcrService(): Promise<OcrProviderStatus> {
  const current = await getHunyuanOcrStatus();
  if (!current.online) return current;
...
export async function recognizeImageWithHunyuan(image: Uint8Array, {
  fetchImpl = fetch,
  timeoutMs = Number(process.env.HUNYUAN_OCR_TIMEOUT_MS || 90000)
}: HunyuanFetchOptions = {}): Promise<RecognizedOcrText> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    ...
    const body = await response.json().catch(() => ({}));
    const errorMessage = body && typeof body === "object" && "error" in body && body.error && typeof body.error === "object" && "message" in body.error
      ? String(body.error.message)
      : response.statusText;
    if (!response.ok) throw new Error(`HunyuanOCR HTTP ${response.status}: ${errorMessage}`);
...
export async function auditPdfWithHunyuanOcr(buffer: Buffer, {
  layout = null,
  totalPages = 0,
  pageNumbers = [],
