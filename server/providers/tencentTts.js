...
async function requestTencentTts({ body, secretId, secretKey, timestamp }) {
...
export async function synthesizeWithTencent({ text, sentenceId, voice }) {
  ...
  try {
    ...
    console.warn(
      `[tts] provider=tencent voice=${baseBody.VoiceType} model=${baseBody.ModelType} sentence=${sentenceId} enableSubtitle=true failed="${error.message}" fallback=true`
    );
