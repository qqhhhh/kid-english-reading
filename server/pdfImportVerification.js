...
import { auditPdfWithXfyunPdfOcr } from "./providers/xfyunPdfOcr.js";
import { auditPdfWithHunyuanOcr, probeHunyuanOcr } from "./providers/hunyuanOcr.js";
import { reviewPdfWithSenseNova } from "./providers/sensenovaVision.js";
...
function buildCloudPageSelection(localAudit, totalPages) {
...
export function describeOptionalProviderFailure(providerName, error) {
...
export function optionalProviderErrorDetail(error) {
...
function shouldRetryOptionalProvider(error) {
...
async function runOptionalProvider(operation) {
  try {
...
function unavailableProvider(engine, error, totalPages) {
  ...
  return {
    ...
    engine,
    ...(engine === "hunyuan-ocr-local" ? { advisory: true } : {}),
    detail: optionalProviderErrorDetail(error),
    totalPages
  };
}

export function mergeVerificationAudits(localAudit, providerAudits = [], visualReview = null) {
...
export async function verifyPdfImport(buffer, { layout = null, totalPages = 0 } = {}) {
  ...
  if (providers.has("xfyun-pdf")) {
    providerTasks.push(runOptionalProvider(() => auditPdfWithXfyunPdfOcr(buffer, { layout, totalPages: pageCount, pageNumbers: cloudPages }))
      .catch((error) => unavailableProvider("xfyun-pdf-ocr", error, pageCount)));
  }
  const visualTask = providers.has("sensenova")
    ? reviewPdfWithSenseNova(buffer, {
        layout,
        totalPages: pageCount,
        pageNumbers: [...new Set([...(localAudit.reviewPages || []), ...(localAudit.criticalPages || []), ...cloudPages])]
      }).catch((error) => ({
        status: "unavailable",
        engine: "sensenova-vision",
        model: process.env.SENSENOVA_MODEL || "",
        message: describeOptionalProviderFailure("日日新视觉复核", error),
        detail: optionalProviderErrorDetail(error),
        pagesProcessed: 0,
        totalPages: pageCount,
        pages: []
  ...
  const [providerAudits, visualReview] = await Promise.all([Promise.all(providerTasks), visualTask]);
  return mergeVerificationAudits(localAudit, providerAudits, visualReview);
}
