...
import type { OcrAuditOptions } from "./types/ocr.js";
...
interface TesseractResultDataLike {
  text?: unknown;
  blocks?: Array<{
    paragraphs?: Array<{
...
interface PdfTextLayerPageLike {
  lines?: Array<{ text?: unknown }>;
}

interface ComparePdfAndOcrInput {
  pageNumber: number;
  confidence: number;
  ocrText: string;
  ocrLines?: string[] | null;
  layoutPage?: PdfTextLayerPageLike | null;
}

interface OcrAuditSummaryOptions {
  engine?: string;
  model?: string;
  totalPages?: number;
  message?: string;
...
function normalizeEnglishText(value: unknown = ""): string {
  return String(value || "")
    .normalize("NFKC")
    .replace(/[\u2018\u2019\u02bc]/g, "'")
    .replace(/[^A-Za-z0-9']+/g, " ")
...
function splitEnglishLines(value: unknown = ""): string[] {
  return String(value || "")
    .split(/\r?\n/)
...
function getConfidentOcrLines(resultData: TesseractResultDataLike): string[] {
...
function countTokenIntersection(leftTokens: readonly string[], rightTokens: readonly string[]): number {
...
function lineSimilarity(left: unknown, right: unknown): number {
...
function findBestMatch(line: string, candidates: readonly string[]): { similarity: number; text: string } {
...
function summarizeUnmatched(lines: readonly string[], candidates: readonly string[]) {
  return lines
...
function getPdfEnglishLines(layoutPage?: PdfTextLayerPageLike | null): string[] {
  return (layoutPage?.lines || [])
    .map((line) => String(line.text || "").replace(/\s+/g, " ").trim())
    .filter((line) => englishWordPattern.test(line));
}

export function comparePdfAndOcrPage({
  pageNumber,
  confidence,
  ocrText,
  ocrLines: providedOcrLines,
  layoutPage
}: ComparePdfAndOcrInput): OcrAuditPage {
...
export function buildUnavailableOcrAudit(message = "OCR unavailable"): OcrAudit {
  return {
    status: "unavailable",
...
export function summarizeOcrAuditPages(pages: readonly OcrAuditPage[], {
  engine = "unknown",
  model = "",
  totalPages = pages.length,
  message = ""
}: OcrAuditSummaryOptions = {}): OcrAudit {
...
export async function auditPdfWithOcr(buffer: Buffer, {
  layout = null,
  totalPages = 0,
      ...
      await Promise.all(workers.map(async (worker, workerIndex) => {
        for (let index = workerIndex; index < renderedPages.length; index += workers.length) {
          ...
          pages.push(comparePdfAndOcrPage({
            ...
            ocrLines: getConfidentOcrLines(result.data as TesseractResultDataLike),
