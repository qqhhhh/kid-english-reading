...
function isIgnorableLayoutLine(text) {
...
function collectStructureSources(structure) {
...
function lineMatchesClassifiedText(lineText, classifiedTexts) {
...
function assessLayoutCoverage(layout, structure) {
...
function assessStructureConsistency(chapters, structure) {
...
function findSentenceSection(chapter, sentenceId) {
  return (chapter.sections || []).find((section) =>
    (section.sentences || []).some((sentence) => sentence.id === sentenceId)
  );
}

function addIssue(issues, issueSentenceIds, input) {
  issues.push({
...
export function assessPdfImportQuality(chapters = [], context = {}) {
  const issues = [];
  const issueSentenceIds = new Set();
...
export function getPdfPublicationBlockers(report) {
