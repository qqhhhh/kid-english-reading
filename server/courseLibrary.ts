...
export interface CourseLibrarySentence {
  id?: string;
  text: string;
  itemType?: string;
  phonetic?: string;
...
export interface CourseLibrarySection {
  id?: string;
  title?: string;
  type?: string;
  partKind?: string;
  ...
  sentences?: CourseLibrarySentence[];
  [key: string]: unknown;
}

export interface CourseLibraryChapter {
  id?: string;
  title?: string;
  sections?: CourseLibrarySection[];
  [key: string]: unknown;
}
...
export interface CourseLibraryResource {
  ...
  tags: string[];
  sourceLabel?: string;
  chapters?: CourseLibraryChapter[];
  content?: { chapters?: CourseLibraryChapter[]; [key: string]: unknown };
  [key: string]: unknown;
}
...
function countResource(resource: CourseLibraryResource) {
  const chapters = resource.chapters || resource.content?.chapters || [];
...
export function cloneCourseLibraryResource(resourceId: string, minScore = 75) {
  const resource = resources.find((item) => item.id === resourceId);
  if (!resource) return null;
  return cloneCourseLibrarySnapshot(resource, minScore);
}

export function cloneCourseLibrarySnapshot(resource: CourseLibraryResource, minScore = 75) {
