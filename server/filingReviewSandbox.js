const sentence = (id, text, minScore = 75) => ({ id, text, minScore, required: true, itemType: "sentence" });
...
export function isFilingReviewSentenceText(text) {
  return filingReviewSentenceTexts.has(String(text || "").trim());
}

export function findFilingReviewSentence(sentenceId) {
  return filingReviewLessons[0].sentences.find((item) => item.id === String(sentenceId || "")) || null;
}

export function sendFilingReviewReadModel(request, response) {
