export function getAssessmentRejection({ referenceText, result, gate, recordingQuality }) {
  const rms = Number(recordingQuality?.rms);
  if (!Number.isFinite(rms)) return null;

  const words = Array.isArray(result?.Words) ? result.Words : [];
  const referenceWordCount = Math.max(1, countReferenceWords(referenceText), words.filter((word) => word.MatchTag !== 1).length);
...
function countReferenceWords(text) {
