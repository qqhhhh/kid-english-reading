...
- 2026-07-16 已按完整数据备份、编译产物健康检查和失败自动回滚流程，将迁移后的编译入口部署到 `qiangzihang.com`，功能提交为 `3de25b2`，正式构建号为 `20260716-1156`。
...
- 只有用户再次明确授权部署时，才备份正式 `server/data/` 并更新 `qiangzihang.com`。
