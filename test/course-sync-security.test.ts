...
import { isRecord, responseJson } from "../test-support/helpers.js";
...
function startServer(context: TestContext): Promise<number> {
...
function buildPackage(): PackagePayload {
...
function signedHeaders(packageHash: string, nonce: string, timestamp: string): SignedHeaders {
  return {
...
function packageForm({ manifestRaw, png }: PackagePayload): FormData {
...
test("signed course packages are accepted once and replay is rejected before parsing", async (context) => {
  ...
  assert.equal((await responseJson<ErrorResponse>(replay)).error, "COURSE_SYNC_REPLAY_REJECTED");
  const logs = listPlatformAdminAuditLogs({ limit: 20 }).filter((entry) => entry.action === "course.sync.receive");
  assert.ok(logs.some((entry) => entry.status === "success" && isRecord(entry.metadata) && entry.metadata.keyId === "primary"));
  assert.ok(logs.some((entry) => entry.status === "failure" && isRecord(entry.metadata) && entry.metadata.statusCode === 409));
...
test("course sync rejects oversized requests from headers before reading a body", async (context) => {
  const port = await startServer(context);
  const result = await new Promise<RawHttpResponse>((resolve, reject) => {
