...
import type { PdfImportParserResult } from "../server/pdfImportParser.js";
import type { PdfLayout, PdfLayoutItem, PdfLayoutLine, PdfLayoutPage } from "../server/types/pdf.js";

type LineEntry = string | { text: string; top: number; x?: number };
type ParsedChapter = PdfImportParserResult["chapters"][number];
...
function makeLine(text: string, top: number, x = 72): PdfLayoutLine {
...
function makePage(page: number, entries: LineEntry[]): PdfLayoutPage {
  const lines = entries.map((entry, index) =>
    makeLine(
      typeof entry === "string" ? entry : entry.text,
      typeof entry === "string" ? 60 + index * 34 : entry.top,
      typeof entry === "string" ? 72 : entry.x ?? 72
    )
  );
  return {
    page,
...
function makeFragmentItem(text: string, x: number, top: number, index: number): PdfLayoutItem {
...
test("Reading time keeps one paragraph per illustrated panel", () => {
  const paragraphs = mergePepReadingParagraphs([
    { text: "My mum is a writer.", page: 17, panelNumber: 1, layout: { ...makeLine("My mum is a writer.", 120), page: 17 } },
    { text: "She writes a lot of good books.", page: 17, panelNumber: 1, layout: { ...makeLine("She writes a lot of good books.", 145), page: 17 } },
    { text: "Mum is also a great cook.", page: 17, panelNumber: 2, layout: { ...makeLine("Mum is also a great cook.", 260), page: 17 } }
  ]);
  ...
  assert.ok(Number(paragraphs[0]?.layout.bottom) >= 162);
});

function makeFragmentRow(parts: string[], startX: number, top: number, startIndex: number, topOffsets: number[] = []): PdfLayoutItem[] {
...
function makeFragmentedWeatherPage(): PdfLayoutPage {
  const lines = [
...
function makeVocabularyItem(text: string, x: number, top: number, index: number, fontName = "pep-regular"): PdfLayoutItem {
  return {
...
function makeRequiredVocabularyRow(parts: string[], x: number, top: number, startIndex: number): PdfLayoutItem[] {
  const items: PdfLayoutItem[] = [makeVocabularyItem("*", x, top, startIndex, "pep-regular")];
  let nextX = x + 8;
...
function makeVocabularyPage(page: number, lineEntries: LineEntry[], items: PdfLayoutItem[]): PdfLayoutPage {
  const lines = lineEntries.map((entry, index) =>
    makeLine(
      typeof entry === "string" ? entry : entry.text,
      typeof entry === "string" ? 40 + index * 28 : entry.top,
      typeof entry === "string" ? 72 : entry.x ?? 72
    )
  );
  return {
...
function makePepRegressionLayout(): PdfLayout {
  ...
  return {
    ...
    stats: {
      ...
      overlayAlternatives: 0
    }
  };
}

function requiredChapter(result: PdfImportParserResult, index: number): ParsedChapter {
...
test("PEP layout keeps all six TOC units and excludes revision/appendix content", () => {
  ...
  const firstChapter = requiredChapter(result, 0);
  assert.ok(firstChapter.leadIn);
  assert.ok(firstChapter.parts);
  assert.ok(firstChapter.sections);
  ...
  assert.equal(firstChapter.leadIn.label, "Lead-in");
  assert.deepEqual(firstChapter.leadIn.activities.map((activity) => activity.title), ["Listen and chant"]);
  assert.deepEqual(
    firstChapter.parts.map((part) => ({
      label: part.label,
  ...
  const wordsSection = firstChapter.sections[0];
  assert.ok(wordsSection);
  assert.equal(wordsSection.title, "Words");
  assert.equal(wordsSection.partKind, "vocabulary");
  assert.deepEqual(
    wordsSection.sentences.map((sentence) => sentence.text),
    ["job", "doctor", "office worker"]
  );
  assert.equal(wordsSection.sentences[0]?.required, false);
  assert.equal(wordsSection.sentences[1]?.required, true);
  assert.deepEqual(
    firstChapter.sections[1]?.sentences.map((sentence) => sentence.text),
    [
      "Can you help?",
      "Yes, I can. I can clean my room.",
      "Can she help?",
  ...
  assert.deepEqual(
    firstChapter.sections[2]?.sentences.map((sentence) => sentence.text),
    [
      "You have a big family. Is this your father?",
      "Yes, it is. He's a PE teacher.",
      "What's your mother's job?",
  ...
  assert.equal(firstChapter.sentences[0]?.text, "job");
  const readingSection = firstChapter.sections.at(-1);
  assert.ok(readingSection);
  assert.equal(readingSection.title, "Reading time");
  assert.deepEqual(
    readingSection.sentences.map((sentence) => sentence.text),
    ["My mum writes good books.", "She can cook great food!", "We can help her at home.", "We want to make this."]
  );
  const weatherChapter = requiredChapter(result, 4);
  const weatherTalk = weatherChapter.sections?.find((section) => section.partLabel === "A" && section.type === "lets-talk");
...
test("PEP import does not deduplicate the same sentence across different units", () => {
  ...
  const unitTwo = structure.units[1];
  assert.ok(unitTwo);
  const unitTwoChant = unitTwo.sections.find((section) => section.activityKey === "listen-and-chant");
  assert.ok(unitTwoChant);
  const firstBlock = unitTwoChant.blocks[0];
  assert.ok(firstBlock);
  firstBlock.text = "Can you help?";
  firstBlock.sentences = ["Can you help?"];

  const result = buildPdfImportChaptersFromStructure({ structure, sourceMode: "layout-structure" });
  assert.equal(requiredChapter(result, 0).sentences.some((sentence) => sentence.text === "Can you help?"), true);
  assert.equal(requiredChapter(result, 1).sentences.some((sentence) => sentence.text === "Can you help?"), true);
});
...
test("PEP Part hierarchy survives SQLite storage and hydration", () => {
  ...
  const result = buildPdfImportChaptersFromStructure({ structure, sourceMode: "layout-structure" });
  const sourceChapter = requiredChapter(result, 0);
  assert.ok(sourceChapter.sections);
  ...
  const scoredSentence = (sentenceId: string) => {
    const sentence = withScores.get(sentenceId);
    assert.ok(sentence);
    return sentence;
  };
  const chapter = {
    ...
    body: sourceChapter.text,
    leadIn: sourceChapter.leadIn,
    parts: sourceChapter.parts,
    sentences: sourceChapter.sentences.map((sentence) => scoredSentence(sentence.id)),
    sections: sourceChapter.sections.map((section) => ({
      ...
      sentences: section.sentences.map((sentence) => scoredSentence(sentence.id))
    }))
  };
  ...
  assert.ok(saved);
  const savedChapter = saved.chapters[0];
  assert.ok(savedChapter);
  const savedWords = savedChapter.sections[0];
  assert.ok(savedWords);
  assert.equal(savedWords.title, "Words");
  assert.deepEqual(savedWords.sentences.map((sentence) => sentence.text), ["job", "doctor", "office worker"]);
  assert.equal(savedWords.sentences[0]?.required, false);
  assert.equal(savedWords.sentences[1]?.itemType, "word");
  assert.equal(savedWords.sentences[1]?.phonetic, "/test/");
  const savedReading = savedChapter.sections.at(-1);
  assert.ok(savedReading);
  assert.equal(savedReading.type, "reading-time");
  assert.equal(savedReading.sentences[0]?.itemType, "reading");
  assert.equal(savedReading.sentences[0]?.panelNumber, 1);
  assert.ok(savedChapter.leadIn);
  assert.equal(savedChapter.leadIn.activities[0]?.title, "Listen and chant");
  assert.deepEqual(
    savedChapter.parts.map((part) => ({
      label: part.label,
      focusQuestion: part.focusQuestion,
      activities: part.activities.map((activity) => activity.title)
    })),
