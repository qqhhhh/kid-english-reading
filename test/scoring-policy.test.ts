...
test("any missed word forces the displayed and stored total score to zero", () => {
  const result = applyScorePolicy<AssessmentResultLike>({
    SuggestedScore: 83,
    Words: [
...
test("low scoring suffix words stay matched unless the provider explicitly marks them missed", () => {
  const result = applyScorePolicy<AssessmentResultLike>({
  ...
  assert.equal(result.SuggestedScore, 49.96);
  assert.equal(result.ProviderSuggestedScore, 49.96);
  assert.equal(result.PronCompletion, 1);
  assert.equal(result.ProviderPronCompletion, undefined);
  assert.deepEqual(
