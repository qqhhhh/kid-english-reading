...
interface PublishedResourceResponse {
  version: number;
...
test("platform admin publishes a PDF preview without creating a household lesson", async (context) => {
  ...
  const registrationBody = await responseJson<RegistrationResponse>(registration);
  ...
  const resource = JSON.parse(publishedText) as PublishedResourceResponse;
  ...
  const householdLessons = await fetch(`${base}/api/admin/lessons`, { headers: { Cookie: cookie } }).then((response) => responseJson<unknown[]>(response));
